...
export class MgOperandoEyComponent implements OnInit {
  ...
  tituloReporte = 'Reporte MG Operando'
   ...
   showDetalle: Show = { toolbar: true, footer: true, header: false, toolbarColumns: false, toolbarInput: false, toolbarSearch: false, toolbarReload: false }
  ...
  getDataRegiones(pais: string) {
    ...
    this.regiones = []
    this._faultMgOperando.getDataRegiones(pais).subscribe((data: any) => {
      data.map(region => {
  ...
  getTableDetalle(search?) {
    this._mgOperandoEyService.getTableDetalle(search).subscribe(({data, message}: any) => {
